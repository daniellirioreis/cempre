        ...
        course = Course.find_by_name(row["NIVEL"].upcase)
         if course.present?
            ...
            unless teacher.present?
              teacher = Teacher.new(name: row["PROFESSOR"].upcase, company_id: Company.last.id)
              teacher.save
            end
            ...
            new_classroom = Classroom.new( name: row["TURMA"].upcase, company_id: Company.last.id, course_id: course.id , teacher_id: teacher.id, day_week: day, capacity: 15, time_start: time_start, time_end: time_end, calendar_id: @calendar.id)
