namespace :cancan do
  ...
  task :create_roles => :purge_roles do
    Role.transaction do
      ...
      yaml['actions_more_questionnaires'].each do |action|
        r = Role.find_by_action_and_controller(action, 'questionnaires')
        unless r.present?
          Role.create(:controller => 'questionnaires', :action => action)
        end
      end
    end
  end
